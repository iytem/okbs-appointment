<?= breadcrumbs("Takip Dosya İşlemleri","fa-search-plus",array("Takip Dosya İşlemleri","Liste"))?>
                ...
                <div class="box-header with-border">
                    <h1 class="box-title" style="color: tomato"><i
                        ...
                        <a class="btn btn-flat btn-bitbucket btn_add_new btn-sm pull-right" id="btn_add_new"
                           data-toggle="tooltip" data-placement="top" title="Yeni Dosya"
                           href="javascript:void(0)" onclick="generate_modal('dosya_add','<?= $this->uri->segment(3)?>','Dosya Ekleme')"><i
                                    class="fa fa-plus-square-o"></i> <span class="hidden-xs hidden-sm">Dosya Ekle</span></a>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": BASE_URL + "takip/dosya_grid_view/"+'<?=$this->uri->segment(3)?>',
                "type": "POST",
                "data": function (data) {
    ...
    function generate_modal(par, id, title) {
        $.ajax({
            ...
            success: function (res) {
                ...
                }else if (par == "dosya_delete") {
                    $('.modal-dialog').addClass('modal-sm');
                    $('.submit').html('<button type="button" class="btn btn-danger" onclick="submit_delete();"><i class="fa fa-trash-o"></i> Sil</button>');
                }  else {
                    $('.modal-dialog').removeClass('modal-sm');
                    $('.submit').html('');
                }
    ...
    function submit_delete() {
        $('.loading').show();
        var form_takip_dosya = $('#form_takip_dosya');
        var data_post = form_takip_dosya.serializeArray();
