...
<?= form_open(base_url('randevu/edit_save/' . $randevu->randevu_id), [
    'name' => 'form_randevu',
...
<div class="form-group ">
    <div class="col-sm-6 col-xs-6">
        <input type="text" class="form-control" name="randevu_ad_soyad" id="randevu_ad_soyad" placeholder="Ad ve Soyad"
               value="<?= set_value('randevu_ad_soyad', $randevu->randevu_ad_soyad); ?>">
    </div>
    <div class="col-sm-6 col-xs-6">
        <input type="text" class="form-control" name="randevu_calistigi_yer" id="randevu_calistigi_yer"
               placeholder="Çalıştığı Yer"
               value="<?= set_value('randevu_calistigi_yer', $randevu->randevu_calistigi_yer); ?>">
    </div>
</div>
...
<div class="form-group ">
    ...
    <div class="col-sm-12">
        <input type="text" class="form-control" name="randevu_telefon_no" id="randevu_telefon_no"
               placeholder="Telefon Numarası"
               value="<?= set_value('randevu_telefon_no', $randevu->randevu_telefon_no); ?>">
    </div>
...
<div class="form-group ">
    ...
    <div class="col-sm-12">
        <input type="text" class="form-control" name="randevu_sebep" id="randevu_sebep" placeholder="Randevu Sebebi"
               value="<?= set_value('randevu_sebep', $randevu->randevu_sebep); ?>">
    </div>
</div>
...
<div class="form-group ">
    ...
    <label for="name" class="col-sm-6 col-xs-6">Saat </label>
...
<div class="form-group ">
    <div class="col-sm-6 col-xs-6">
        <input type="text" class="form-control dateInput" name="randevu_tarih" id="randevu_tarih" placeholder="Tarih"
               value="<?= set_value('randevu_tarih', $randevu->randevu_tarih); ?>">
    </div>
    <div class="col-sm-6 col-xs-6">
        <input type="text" class="form-control time" name="randevu_saat" id="randevu_saat" placeholder="Saat"
               value="<?= set_value('randevu_saat', $randevu->randevu_saat); ?>">
    </div>
</div>
<div class="form-group ">
    <label for="content" class="col-sm-6">Randevu Durum <i class="required">*</i></label>
    <div class="col-sm-12">
        <select class="form-control chosen  chosen-select-deselect" name="randevu_durum" id="randevu_durum"
                tabi-ndex="5" data-placeholder="Seçiniz">
            <option value=""></option>
            <option value="1" <?= $randevu->randevu_durum == 1 ? "selected" : "" ?>>Henüz Gerçekleşmedi</option>
            <option value="2" <?= $randevu->randevu_durum == 2 ? "selected" : "" ?>>Randevu Gerçekleştirildi</option>
        </select>
    </div>
</div>
        ...
        <div class="col-sm-12">
            <select class="form-control chosen " name="randevu_birim" id="randevu_birim"
                ...
                <?php foreach (db_get_all_data('okbs_birim', ['birim_durum' => 1]) as $row): ?>
                    <option value="<?= $row->birim_id; ?>" <?= $randevu->randevu_birim == $row->birim_id ? "selected" : "" ?>><?= ucwords($row->birim_adi); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
<?php else: ?>
    <input type="hidden" class="form-control" name="randevu_birim" id="randevu_birim" placeholder="Birim"
           value="<?= set_value('randevu_birim', $randevu->randevu_birim); ?>">
<?php endif; ?>


<div class="form-group ">
    <label for="definition" class="col-sm-3">Görüşme Notlar <i
